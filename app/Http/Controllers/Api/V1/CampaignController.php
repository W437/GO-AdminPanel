...
class CampaignController extends Controller
{
    ...
    public function get_item_campaigns(Request $request){
        ...
        try {
            // Get active campaigns running in the specified zone
            $campaigns = ItemCampaign::Active()
                ->Running()
                ->whereIn('zone_id', $zone_id)
                ->with(['foods' => function($query) use ($zone_id, $longitude, $latitude) {
                    $query->with(['restaurant' => function($q) use ($zone_id) {
            ...
            // Collect all food items from all active campaigns
            $foodItems = collect();
            foreach ($campaigns as $campaign) {
                $foodItems = $foodItems->merge($campaign->foods);
            }

            // Remove duplicates (same food in multiple campaigns)
            $foodItems = $foodItems->unique('id');

            // Format food items using the standard product formatter
            $formattedItems = Helpers::product_data_formatting($foodItems, true, false, app()->getLocale());
