            ...
            <div class="row g-2">
                            ...
                            @if($language)
                                <div class="lang_form" id="default-form">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="title[]" id="default_title" class="form-control" placeholder="{{translate('messages.campaign_title_placeholder')}}" required>
                                    </div>
                                    <input type="hidden" name="lang[]" value="default">
                                    <div class="form-group">
                                        <label class="input-label" for="default_description">{{translate('messages.description')}} (Default)</label>
                                        <textarea name="description[]" id="default_description" class="form-control h--72px" placeholder="{{translate('messages.campaign_description_placeholder')}}"></textarea>
                                    </div>
                                </div>
                                @foreach(json_decode($language) as $lang)
                                    <div class="d-none lang_form" id="{{$lang}}-form">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="title[]" id="{{$lang}}_title" class="form-control" placeholder="{{translate('messages.campaign_title_placeholder')}}">
                                        </div>
                                        <input type="hidden" name="lang[]" value="{{$lang}}">
                                        <div class="form-group">
                                            <label class="input-label" for="{{$lang}}_description">{{translate('messages.description')}} ({{strtoupper($lang)}})</label>
                                            <textarea name="description[]" id="{{$lang}}_description" class="form-control h--72px" placeholder="{{translate('messages.campaign_description_placeholder')}}"></textarea>
                                        </div>
                                    </div>
                                @endforeach
                            @else
                                <div id="default-form">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="title[]" id="title" class="form-control" placeholder="{{translate('messages.campaign_title_placeholder')}}" required>
                                    </div>
                                    <input type="hidden" name="lang[]" value="default">
                                    <div class="form-group">
                                        <label class="input-label" for="description">{{translate('messages.description')}}</label>
                                        <textarea name="description[]" id="description" class="form-control h--72px" placeholder="{{translate('messages.campaign_description_placeholder')}}"></textarea>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <center class="mb-4">
                                <img class="initial-14 border rounded" id="viewer"
                                    src="{{dynamicAsset('public/assets/admin/img/upload-img.png')}}" alt="campaign image"/>
                            </center>
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="alert alert-info" role="alert" id="zone-alert">
                                <i class="tio-info-outined"></i> {{translate('messages.please_select_a_zone_first_to_load_food_items')}}
                            </div>
                            <div class="form-group d-none" id="food-select-container">
                                <label class="input-label" for="food_ids">{{translate('messages.food_items')}}<span class="text-danger">*</span></label>
                                <select name="food_ids[]" id="food_ids" class="form-control" multiple="multiple" required disabled>
                                    <option value="">{{translate('messages.select_zone_first')}}</option>
                                </select>
                                <small class="text-muted">{{translate('messages.select_multiple_food_items_for_this_campaign')}}</small>
                            </div>
                            <div id="selected-foods-preview" class="mt-3"></div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    // Zone selection and food loading
    $('#zone_id').on('change', function() {
        let zoneId = $(this).val();
        if (zoneId) {
            loadFoodItems(zoneId);
        } else {
            $('#food_ids').prop('disabled', true).html('<option value="">{{translate('messages.select_zone_first')}}</option>');
            $('#food-select-container').addClass('d-none');
            $('#zone-alert').removeClass('d-none');
        }
    });

    function loadFoodItems(zoneId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    ...
                    $('#food-select-container').removeClass('d-none');
                    $('#zone-alert').addClass('d-none');
